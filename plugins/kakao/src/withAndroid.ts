 ...
 */
const withKakaoInternetPermission: ConfigPlugin = (config) => {
 ...
 */
const withKakaoNativeAppKey: ConfigPlugin<{ nativeAppKey: string }> = (
	config,
	{ nativeAppKey },
) => {
	return withAndroidManifest(config, (config) => {
 ...
 */
export const withAndroid: ConfigPlugin<{ nativeAppKey: string }> = (
	config,
	{ nativeAppKey },
) => {
	if (!nativeAppKey) {
		throw new Error(
			"[Kakao Android Plugin] 'nativeAppKey' is required. Please provide a valid Kakao Native App Key.",
		);
	}
